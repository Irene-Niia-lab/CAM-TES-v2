...
import { BarChart3, TrendingUp, Users, Award, Calculator, Hash, FileDown, MessageSquareText, ChevronDown, ChevronUp, User, Tag, Building2, FileSpreadsheet, Edit3, Save, Trash2, AlertTriangle } from 'lucide-react';
...
const StatisticsView: React.FC<StatisticsViewProps> = ({ candidates, onDeleteGroup, onClearAll }) => {
  ...
  const handleClearAllClick = () => {
    if (window.confirm("❗ 警告：此操作将彻底清除【全场所有评委】上传的所有评分记录！\n该操作不可撤销，请确保已导出备份后再执行。确定继续吗？")) {
      onClearAll();
    }
  };
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <button 
            ...
            className="flex items-center justify-center gap-2 px-6 py-4 bg-red-50 text-red-600 rounded-2xl font-black text-sm hover:bg-red-100 transition-all border border-red-100 disabled:opacity-50 disabled:cursor-not-allowed group"
          >
            <Trash2 className="w-5 h-5 group-hover:shake" />
            清除全员记录
          </button>
          <button onClick={() => exportToExcel(candidates)} className="flex-1 md:flex-none flex items-center justify-center gap-3 px-8 py-4 bg-emerald-600 text-white rounded-2xl font-black text-lg hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-100 active:scale-95 group">
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm flex-1 overflow-hidden flex flex-col">
        {candidates.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center text-slate-300">
            ...
            <p className="text-lg font-bold">暂无评分数据，请通过侧边栏添加或管理员导入</p>
          </div>
        ) : (
